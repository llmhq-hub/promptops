...
- `GitVersioning.is_git_repo()` no longer reports a missing git binary as "not a repository". Because `PromptOpsError` subclasses `ValueError` (the v0.4.0 back-compat decision), its bare `except ValueError` was swallowing E018 and sending callers to look for a `.git/` that was present all along. Worth noting as a general hazard of that subclassing choice: any `except ValueError` that converts to a boolean rather than re-raising can silently absorb a structured error.
